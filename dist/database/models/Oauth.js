...
const getAuthorizationCode = async authorizationCode => {
  const authCode = await _OAuthAuthorizationCode.default.findOne({
    authorizationCode
...
const getUser = async _id => {
  const user = await _OAuthUser.default.findOne({
    _id
  }).lean();
  return user;
};
...
const revokeAuthorizationCode = async code => {
  await _OAuthAuthorizationCode.default.deleteMany({
    authorizationCode: code.authorizationCode
  });
  return true;
};
